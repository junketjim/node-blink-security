...
module.exports = class Blink {
  ...
  isArmed() {
    return this.getSummary().then((summaries) => {
  ...
  getVideos(page = 0, since = new Date(2008)) {
  ...
  setupSystem(name_or_id) {
    if (!((this._username && this._password) || (this._token && this._region_id))) {
      throw new BlinkAuthenticationException(
        "(_username, _password) or (_token, _region_id) are required for system setup"
      );
    }
    if (this._token) {
      this._setupWithToken();
      return this.getIDs(name_or_id).then(this.getCameras.bind(this)).then(this.getLinks.bind(this));
    } else {
        ...
        (err, response, body) => {
          if (err || _statusCodeIsError(response)) {
            ...
            if ((body.client || {}).verification_required) {
              if (!this.auth_2FA) {
                if (repeats === 1)
                  return reject(new BlinkAuthenticationException(`Authentication problem: verification timeout`));
                return new Promise((resolve, reject) => {
                  setTimeout(() => {
